...
## Package Issue Reporting

### Overview

When encountering limitations, bugs, or shortcomings in **our own packages** (packages under the `vnykmshr` GitHub organization), we maintain a policy of creating detailed GitHub issues to track and evaluate potential improvements.

### When to Create Issues

Create a GitHub issue when you encounter:

1. **Bugs**: Unexpected behavior or errors in package functionality
2. **Limitations**: Missing features that would benefit the project
3. **Integration Challenges**: Difficulties integrating the package with standard Go patterns
4. **API Design Issues**: Confusing or restrictive API design
5. **Documentation Gaps**: Missing or unclear documentation
6. **Performance Problems**: Bottlenecks or inefficient implementations

### Our Packages

Current packages to monitor:
- **gopantic** ([vnykmshr/gopantic](https://github.com/vnykmshr/gopantic)) - Validation library with Pydantic-style API
...
- Environment details (Go version, package version)

#### 2. Create Detailed GitHub Issue

Use the `gh` CLI for quick issue creation:

```bash
# Example: Creating an issue for gopantic
gh issue create \
  --repo vnykmshr/gopantic \
  --title "Bug: json.RawMessage Fields Cause Parse Errors" \
  --body-file /path/to/detailed-description.md \
  --label bug,enhancement
```

Or create via web interface with this template:
...
When implementing workarounds in Nivo, add a comment referencing the issue:

```go
// Workaround for gopantic issue #10: json.RawMessage incompatibility
// TODO: Remove this workaround once gopantic supports json.RawMessage
...
See these examples of well-documented package issues:

1. **gopantic #10**: [json.RawMessage incompatibility](https://github.com/vnykmshr/gopantic/issues/10)
   - Clear reproduction steps
   - Impact analysis
   - Suggested fix
   - Workaround documented

2. **gopantic #11**: [Separate Unmarshal and Validate functions](https://github.com/vnykmshr/gopantic/issues/11)
   - Use case examples
   - API proposal
   - Migration path

3. **gopantic #12**: [Documentation for complex types](https://github.com/vnykmshr/gopantic/issues/12)
   - Documentation gaps identified
   - Structured examples provided
   - Clear benefit analysis

### Evaluation Criteria

Not every issue needs immediate action. Issues are evaluated based on:

- **Impact**: How many projects/features are affected?
- **Severity**: Does it block development or just cause inconvenience?
- **Workaround**: Is there a reasonable workaround?
- **Maintenance**: Will fixing this simplify future development?
- **Standards Compliance**: Does it align with Go community best practices?

### Integration with Development Workflow

```bash
# When encountering a package issue during development:

# 1. Document the issue locally
echo "Issue found in gopantic: json.RawMessage incompatibility" >> /tmp/package-issues.md

# 2. Implement workaround with TODO comment
# (Add workaround code with reference comment)

# 3. Create detailed GitHub issue
gh issue create --repo vnykmshr/gopantic --title "..." --body-file issue.md

# 4. Reference in commit message
git commit -m "fix: Workaround gopantic json.RawMessage issue (#10)"

# 5. Link issue in PR description
# "This PR implements a workaround for vnykmshr/gopantic#10"
```

### Benefits

This practice ensures:
- ✅ **Continuous Improvement**: Package issues are tracked and evaluated
- ✅ **Knowledge Sharing**: Team members can see known limitations
- ✅ **Better Planning**: Technical debt is visible and quantified
- ✅ **Community Benefit**: Other users benefit from detailed issue reports
- ✅ **Decision Making**: Clear data for "fix vs workaround" decisions

### Guidelines

**DO:**
- Create issues promptly when discovered
- Provide minimal reproduction examples
- Suggest potential solutions
- Document workarounds in code comments
- Link issues in related PRs

**DON'T:**
- Create duplicate issues (search first)
- Include sensitive data in public issues
- Demand immediate fixes (evaluate priority first)
- Skip workarounds while waiting for fixes
